...
{
	const auto& d3dDevice = EngineObj().GetGraphicsEngine()->GetD3DDevice();
	...
	for (auto& descriptorHeap : m_descriptorHeap) {
		...
		for (int i = 0; i < m_numConstantBuffer; i++) {
			...
			//次に進める。
			cpuHandle.ptr += EngineObj().GetGraphicsEngine()->GetCbrSrvDescriptorSize();
		}
		...
		m_srGpuDescriptorStart[bufferNo].ptr += (UINT64)EngineObj().GetGraphicsEngine()->GetCbrSrvDescriptorSize() * m_numConstantBuffer;
		//UAVリソースのディスクリプタヒープの開始ハンドルを計算。
		m_uavGpuDescriptorStart[bufferNo] = gpuHandle;
		m_uavGpuDescriptorStart[bufferNo].ptr += (UINT64)EngineObj().GetGraphicsEngine()->GetCbrSrvDescriptorSize() * ( m_numShaderResource + m_numConstantBuffer );
		bufferNo++;
	}
}
