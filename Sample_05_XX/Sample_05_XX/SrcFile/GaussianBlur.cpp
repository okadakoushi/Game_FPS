...
void GaussianBlur::Init(Texture* originalTexure, bool isBlurAlpha, bool isDownSample)
{
	m_originalTexture = originalTexure;

	//レンダリングターゲットを初期化。
	InitRenderTarget(isDownSample);
	//スプライトを初期化。
	InitSprite(isBlurAlpha);
}
...
void GaussianBlur::ExecuteOnGPU(RenderContext& rc, float blurPower)
{
	...
	//ドロー。
	m_xBlurSprite.Draw(rc, GraphicsEngineObj()->GetCamera2D().GetViewMatrix(), GraphicsEngineObj()->GetCamera2D().GetProjectionMatrix());
	...
	//ドロー。
	m_yBlurSprite.Draw(rc, GraphicsEngineObj()->GetCamera2D().GetViewMatrix(), GraphicsEngineObj()->GetCamera2D().GetProjectionMatrix());
	//レンダリングターゲットへの書き込み終了待ち。
	rc.WaitUntilFinishDrawingToRenderTarget(m_yBlurRenderTarget);
}

void GaussianBlur::InitRenderTarget(bool isDownSample)
{
...
void GaussianBlur::InitSprite(bool isBlurAlpha)
{
