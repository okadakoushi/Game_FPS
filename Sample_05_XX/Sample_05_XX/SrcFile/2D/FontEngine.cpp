...
{
	auto d3dDevice = GraphicsEngineObj()->GetD3DDevice();
	...
	*/
	...
	//バッチ作成。
	m_spriteBatch = make_unique<DirectX::SpriteBatch>(
		d3dDevice,
		re,
		BatchDesc,
		&viewport
	...
	m_spriteFont = make_unique<DirectX::SpriteFont>(
		d3dDevice,
		re,
	...
	re.End(GraphicsEngineObj()->GetCommandQueue());
}

void FontEngine::BeginDraw()
{
	m_spriteBatch->Begin(
		GraphicsEngineObj()->GetCommandList(),
		/*
	...
	GraphicsEngineObj()->GetCommandList()->SetDescriptorHeaps(1, &m_srvDescriptorHeap);
}
